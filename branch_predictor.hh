...
T clamp(T x) {
  return std::max(std::min(max_, x), 0);
}
...
protected:
  ...
  static constexpr int table_lengths[] = {2,4,8,16,32,48,64,96,128,192,256};  
  ...
  void update_correct(uint64_t addr, uint64_t idx, bool prediction, bool taken);
